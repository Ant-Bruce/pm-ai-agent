...
class PMAIAgentApp {
    ...
    initializeElements() {
        ...
        this.riskScanBtn = document.getElementById('riskScanBtn');
        this.weeklyReportBtn = document.getElementById('weeklyReportBtn');
        this.docQABtn = document.getElementById('docQABtn');
    ...
    bindEvents() {
        ...
        // PM快捷操作按钮
        if (this.riskScanBtn) {
            this.riskScanBtn.addEventListener('click', () => this.triggerPMAction('risk_scan'));
        }
        if (this.weeklyReportBtn) {
            this.weeklyReportBtn.addEventListener('click', () => this.triggerPMAction('report'));
        }
        if (this.docQABtn) {
            this.docQABtn.addEventListener('click', () => this.triggerPMAction('doc_qa'));
        }
    ...
    updateUI() {
        ...
        if (this.messageInput) {
            ...
            this.messageInput.placeholder = '输入消息，Enter 发送...';
        }
    }
    ...
    // 发送PM Agent请求（SSE 流式模式）
    async sendPMRequest(taskType, loadingMessageElement) {
        try {
            const response = await fetch(`${this.apiBaseUrl}/pm`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    ...
                    task_type: taskType,
                    query: this.messageInput ? (this.messageInput.value || '') : ''
                })
            });
            ...
            try {
                while (true) {
                    ...
                    if (done) {
                        // 流结束，更新最终内容
                        if (fullResponse) {
                            console.log('PM Agent 流结束，更新最终内容，长度:', fullResponse.length);
                            this.updatePMResultMessage(loadingMessageElement, fullResponse, []);
                        }
                        break;
                    }
                    ...
                    for (const line of lines) {
                        ...
                        console.log('[PM Agent SSE] 收到行:', line);
                        ...
                        if (line.startsWith('id:')) {
                            ...
                            currentEvent = line.substring(6).trim();
                            console.log('[PM Agent SSE] 事件类型:', currentEvent);
                            continue;
                        } else if (line.startsWith('data:')) {
                            const rawData = line.substring(5).trim();
                            console.log('[PM Agent SSE] 数据:', rawData, ', currentEvent:', currentEvent);
                            ...
                            const processJsonMessages = (data) => {
                                ...
                                if (matches && matches.length > 0) {
                                    console.log('[PM Agent SSE] 匹配到', matches.length, '个JSON对象');
                                    for (const jsonStr of matches) {
                                        try {
                                            ...
                                            if (sseMessage.type === 'content') {
                                                ...
                                                console.log('PM Agent 流完成，最终内容长度:', fullResponse.length);
                                                this.updatePMResultMessage(loadingMessageElement, fullResponse, []);
                                                return true;
                                            } else if (sseMessage.type === 'error') {
                                                throw new Error(sseMessage.data || sseMessage.message || 'PM分析失败');
                                            }
                                        } catch (e) {
                                            if (e.message.includes('PM分析')) throw e;
                                            console.log('[PM Agent SSE] 单个JSON解析失败:', jsonStr);
                                        }
                                    }
                                    if (loadingMessageElement) {
                                        this.updatePMResultStreamContent(loadingMessageElement, fullResponse);
                                    }
                                    return false;
                                }
                                return null;
                                ...
                                try {
                                    ...
                                    if (sseMessage && sseMessage.type) {
                                        if (sseMessage.type === 'content') {
                                            ...
                                            // 处理最终报告事件 - 这是关键！
                                            console.log('PM Agent 最终报告生成，流式输出中...');
                                            const reportText = `\n\n## 🎯 诊断报告\n\n${sseMessage.report || ''}\n`;
                                            fullResponse += reportText;
                                            if (loadingMessageElement) {
                                                this.updatePMResultStreamContent(loadingMessageElement, fullResponse);
                                            ...
                                            // 处理完成事件
                                            console.log('PM Agent 诊断完成，最终内容长度:', fullResponse.length);
                                            if (sseMessage.response) {
                                                fullResponse += `\n\n${sseMessage.response}`;
                                            }
                                            // 使用最终的完整内容更新消息
    ...
    // 更新 PM 流式内容（实时显示）
    updatePMResultStreamContent(messageElement, content) {
        if (!messageElement) return;
        
        // 添加 pm-message 类
        messageElement.classList.add('pm-message');
    ...
    // 更新 PM 结果消息（带折叠详情）
    updatePMResultMessage(messageElement, response, details) {
        console.log('updatePMResultMessage 被调用');
        ...
        if (!messageElement) {
            ...
            return this.addPMResultMessage(response, details);
        }
        ...
        if (details && details.length > 0) {
            // 检查是否已存在详情容器
            let detailsContainer = messageElement.querySelector('.pm-details');
            if (!detailsContainer) {
                detailsContainer = document.createElement('div');
                detailsContainer.className = 'pm-details';
    ...
    // 添加 PM 结果消息（带折叠详情）
    addPMResultMessage(response, details) {
        const messageDiv = document.createElement('div');
        messageDiv.className = 'message assistant pm-message';
    ...
    // 触发PM快捷操作
    async triggerPMAction(taskType) {
        if (this.isStreaming) {
        ...
        const labels = { risk_scan: '正在扫描项目风险...', report: '正在生成项目周报...', doc_qa: '正在查询项目文档...' };
        const loadingMessage = this.addLoadingMessage(labels[taskType] || '分析中...');
        this.currentPMResultMessage = loadingMessage;

        this.isStreaming = true;
        this.updateUI();

        try {
            await this.sendPMRequest(taskType, loadingMessage);
        } catch (error) {
            console.error('PM操作失败:', error);
            if (loadingMessage) {
                const messageContent = loadingMessage.querySelector('.message-content');
                if (messageContent) {
                    messageContent.textContent = '抱歉，操作失败：' + error.message;
                }
            }
        } finally {
            this.isStreaming = false;
            this.currentPMResultMessage = null;
            this.updateUI();
        }
    }
    ...
    showLoadingOverlay(show) {
        if (this.loadingOverlay) {
            if (show) {
                ...
                if (loadingText) loadingText.textContent = 'AI PM 分析中，请稍候...';
