...
.PHONY: help init start stop restart check upload clean up down status wait \
        ...
        start-project stop-project start-knowledge stop-knowledge start-api stop-api status-mcp
...
help:
	...
	@echo "  $(YELLOW)make start-project$(NC)   - 📋 启动 Project MCP 服务"
	@echo "  $(YELLOW)make stop-project$(NC)    - 🛑 停止 Project MCP 服务"
	@echo "  $(YELLOW)make start-knowledge$(NC) - 📚 启动 Knowledge MCP 服务"
	@echo "  $(YELLOW)make stop-knowledge$(NC)  - 🛑 停止 Knowledge MCP 服务"
	...
	@echo "  2. 启动服务:   $(YELLOW)make start$(NC) (自动启动 Project + Knowledge MCP + FastAPI)"
...
# 启动 Project MCP 服务
start-project:
	@echo "$(YELLOW)📋 启动 Project MCP 服务...$(NC)"
	@if pgrep -f "mcp_servers/project_server.py" > /dev/null 2>&1; then \
		echo "$(GREEN)✅ Project MCP 服务已经在运行中$(NC)"; \
	else \
		echo "$(YELLOW)📦 正在启动 Project MCP 服务（后台运行）...$(NC)"; \
		nohup .venv/bin/python mcp_servers/project_server.py > mcp_project.log 2>&1 & \
		echo $$! > mcp_project.pid; \
		sleep 2; \
		if pgrep -f "mcp_servers/project_server.py" > /dev/null 2>&1; then \
			echo "$(GREEN)✅ Project MCP 服务启动成功$(NC)"; \
			echo "$(YELLOW)   PID: $$(cat mcp_project.pid)$(NC)"; \
			echo "$(YELLOW)   URL: http://127.0.0.1:8003/mcp$(NC)"; \
			echo "$(YELLOW)   日志: mcp_project.log$(NC)"; \
		else \
			echo "$(RED)❌ Project MCP 服务启动失败$(NC)"; \
			echo "$(YELLOW)请检查日志: tail -f mcp_project.log$(NC)"; \
		fi; \
	fi

# 启动 Knowledge MCP 服务
start-knowledge:
	@echo "$(YELLOW)📚 启动 Knowledge MCP 服务...$(NC)"
	@if pgrep -f "mcp_servers/knowledge_server.py" > /dev/null 2>&1; then \
		echo "$(GREEN)✅ Knowledge MCP 服务已经在运行中$(NC)"; \
	else \
		echo "$(YELLOW)📦 正在启动 Knowledge MCP 服务（后台运行）...$(NC)"; \
		nohup .venv/bin/python mcp_servers/knowledge_server.py > mcp_knowledge.log 2>&1 & \
		echo $$! > mcp_knowledge.pid; \
		sleep 2; \
		if pgrep -f "mcp_servers/knowledge_server.py" > /dev/null 2>&1; then \
			echo "$(GREEN)✅ Knowledge MCP 服务启动成功$(NC)"; \
			echo "$(YELLOW)   PID: $$(cat mcp_knowledge.pid)$(NC)"; \
			echo "$(YELLOW)   URL: http://127.0.0.1:8004/mcp$(NC)"; \
			echo "$(YELLOW)   日志: mcp_knowledge.log$(NC)"; \
		else \
			echo "$(RED)❌ Knowledge MCP 服务启动失败$(NC)"; \
			echo "$(YELLOW)请检查日志: tail -f mcp_knowledge.log$(NC)"; \
		fi; \
	fi

# 停止 Knowledge MCP 服务
stop-knowledge:
	@echo "$(YELLOW)🛑 停止 Knowledge MCP 服务...$(NC)"
	@if [ -f mcp_knowledge.pid ]; then \
		pid=$$(cat mcp_knowledge.pid); \
		if ps -p $$pid > /dev/null 2>&1; then \
			kill $$pid; \
			echo "$(GREEN)✅ Knowledge MCP 服务已停止 (PID: $$pid)$(NC)"; \
		else \
			echo "$(YELLOW)⚠️  进程不存在 (PID: $$pid)$(NC)"; \
		fi; \
		rm -f mcp_knowledge.pid; \
	else \
		echo "$(YELLOW)⚠️  未找到 mcp_knowledge.pid 文件$(NC)"; \
		pkill -f "mcp_servers/knowledge_server.py" 2>/dev/null && \
			echo "$(GREEN)✅ 已停止所有 Knowledge MCP 进程$(NC)" || \
			echo "$(YELLOW)⚠️  没有运行中的 Knowledge MCP 进程$(NC)"; \
	fi
...
status-mcp:
	...
	@echo ""
	@echo "$(CYAN)Project MCP 服务:$(NC)"
	@if pgrep -f "mcp_servers/project_server.py" > /dev/null 2>&1; then \
		pid=$$(pgrep -f "mcp_servers/project_server.py"); \
	...
	@echo ""
	@echo "$(CYAN)Knowledge MCP 服务:$(NC)"
	@if pgrep -f "mcp_servers/knowledge_server.py" > /dev/null 2>&1; then \
		pid=$$(pgrep -f "mcp_servers/knowledge_server.py"); \
		echo "  状态: $(GREEN)运行中$(NC)"; \
		echo "  PID: $$pid"; \
...
# 停止 Project MCP 服务
stop-project:
	@echo "$(YELLOW)🛑 停止 Project MCP 服务...$(NC)"
	@if [ -f mcp_project.pid ]; then \
		pid=$$(cat mcp_project.pid); \
		if ps -p $$pid > /dev/null 2>&1; then \
			kill $$pid; \
			echo "$(GREEN)✅ Project MCP 服务已停止 (PID: $$pid)$(NC)"; \
		else \
			echo "$(YELLOW)⚠️  进程不存在 (PID: $$pid)$(NC)"; \
		fi; \
		rm -f mcp_project.pid; \
	else \
		echo "$(YELLOW)⚠️  未找到 mcp_project.pid 文件$(NC)"; \
		pkill -f "mcp_servers/project_server.py" 2>/dev/null && \
			echo "$(GREEN)✅ 已停止所有 Project MCP 进程$(NC)" || \
			echo "$(YELLOW)⚠️  没有运行中的 Project MCP 进程$(NC)"; \
	fi
...
clean:  ## 清理临时文件
	...
	rm -f mcp_project.pid mcp_project.log
	rm -f mcp_knowledge.pid mcp_knowledge.log
