...
async def index_directory(directory_path: str = None):
    ...
    try:
        ...
        # 检查路径是否在白名单内（用 relative_to 防止前缀绕过）
        allowed = False
        for allowed_dir in ALLOWED_INDEX_DIRS:
            try:
                ...
                continue
        if not allowed:
            logger.warning(f"拒绝索引非白名单目录: {target}")
            raise HTTPException(status_code=403, detail="目录访问被拒绝")
        ...
        logger.error(f"索引目录失败: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="服务内部错误，请稍后重试")
