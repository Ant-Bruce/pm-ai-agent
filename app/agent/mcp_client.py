...
async def get_mcp_client(
    ...
    """
    获取或初始化带重试功能的 MCP 客户端（全局单例）

    重试拦截器默认启用，自动对所有 MCP 工具调用进行指数退避重试。

    Args:
        servers: MCP 服务器配置，默认使用 DEFAULT_MCP_SERVERS
        tool_interceptors: 额外的自定义工具拦截器列表（会在重试拦截器之后添加）
    ...
    if _mcp_client is None:
        logger.info("初始化全局 MCP 客户端（带重试拦截器）...")
...
def _create_mcp_client(
    ...
    """
    创建 MCP 客户端实例（默认带重试拦截器）

    Args:
        servers: MCP 服务器配置
        tool_interceptors: 额外的工具拦截器列表

    Returns:
        MultiServerMCPClient: 客户端实例
    """
    # 构建拦截器列表：重试拦截器在最前面
    ...
    return MultiServerMCPClient(servers, tool_interceptors=interceptors)  # type: ignore[arg-type]
