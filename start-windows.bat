...
REM 启动 Project MCP 服务
echo [5/6] 启动 Project MCP 服务...
start "Project MCP Server" /min %PYTHON_CMD% mcp_servers/project_server.py
timeout /t 2 /nobreak >nul
echo [成功] Project MCP 服务已启动
echo.

REM 启动 Knowledge MCP 服务
echo [6/6] 启动 Knowledge MCP 服务...
start "Knowledge MCP Server" /min %PYTHON_CMD% mcp_servers/knowledge_server.py
timeout /t 2 /nobreak >nul
echo [成功] Knowledge MCP 服务已启动
echo.

REM 启动 FastAPI 服务
echo [7/8] 启动 PM AI Agent API...
start "PM AI Agent API" %PYTHON_CMD% -m uvicorn app.main:app --host 0.0.0.0 --port 9900
echo [信息] 等待服务启动（15秒）...
...
if errorlevel 1 (
    ...
    REM 调用 API 上传 pm-docs 文档到向量数据库
    echo [8/8] 上传文档到向量数据库...
    for %%f in (pm-docs\*.md) do (
        echo   上传: %%~nxf
